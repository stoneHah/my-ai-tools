...
async def test_submit():
    ...
    # 修改连接方式，使用URI和headers参数
    auth_header = f"Bearer {token}"
    async with websockets.connect(api_url, additional_headers={"Authorization": auth_header}) as ws:
...
async def test_query():
    ...
    async with websockets.connect(api_url, additional_headers=header) as ws:
        await ws.send(full_client_request)
        res = await ws.recv()
